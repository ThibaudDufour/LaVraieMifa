...
window.onload = function() {
    ...
    function stateRemoveCluster(dt) {
        if (animationstate == 0) {
            ...
            $.ajax({
                url : '/games/bubble-shooter/score/current/' + score ,
                type : 'GET',
                dataType : 'json'
            })
            .done(function (output) {
                // Code à jouer en cas d'éxécution sans erreur du script du PHP
                document.getElementById("ScorePerso").innerHTML = output.best_score_perso
    ...
    function render() {
        ...
        if (gamestate == gamestates.gameover) {
            ...
            drawCenterText("Cliquez pour recommencer", level.x, level.y + level.height / 2 + 40, level.width);
        }
    }
