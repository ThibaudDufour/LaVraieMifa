...
class ResetPasswordController extends AbstractController
{
    ...
    public function reset(Request $request, UserPasswordHasherInterface $userPasswordHasher, EntityManagerInterface $entityManager, string $token = null): Response
    {
        ...
        try {
            ...
            $this->addFlash('reset_password_error', sprintf(
                'Il y a eu un problème de validation de votre demande de réinitialisation - <br/>%s',
                $e->getReason()
            ));
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer/*, EntityManagerInterface $entityManager*/): RedirectResponse
    {
        ...
        if (!$user) {
            return $this->redirectToRoute('app_check_email'); //app_check_email
        }
        ...
        try {
            ...
            //
            $this->addFlash('reset_password_error', sprintf(
                 'Il y a eu un problème lors du traitement de votre demande de réinitialisation du mot de passe. - </br>%s',
                 $e->getReason()
            ));

            return $this->redirectToRoute('app_forgot_password_request'); //app_check_email
        }
